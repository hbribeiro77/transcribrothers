"""Regeneração do Markdown de notas de proposta (FAB) via LiteLLM."""
...
async def executar_regeneracao_markdown_notas_proposta_funcionalidade_em_background(
    *,
    job_id: str,
    ...
    try:
        ...
        if not job_steps_indicam_notas_proposta_funcionalidade_transcribrothers(steps):
            raise RuntimeError("Este job não é de notas de proposta (destino_apos_transcricao).")

        snap = steps.get("regeneracao_tutorial_snapshot")
        if not isinstance(snap, dict):
            raise RuntimeError(
                "Snapshot de regeneração ausente. Conclua o pipeline de notas de proposta antes de regenerar."
            )
        ...
        if rels:
            for _t, rel in rels:
                nome = rel.split("/")[-1]
                if nome and not (assets_dir / nome).is_file():
                    raise RuntimeError(
                        f"Imagem referenciada não encontrada no servidor (esperado em assets): {nome}"
                    )
        ...
        steps["pipeline_fase"] = "regenerando_markdown_notas_proposta_litellm"
        ...
        steps["regeneracao_notas_proposta_documento_autonomo_sem_video"] = bool(documento_autonomo_sem_video)
        ...
        md = await gerar_markdown_notas_proposta_funcionalidade_com_litellm_transcribrothers(
            ...
            log_etapa_geracao_tutorial="regeneracao_notas_proposta_markdown",
        )
        ...
        preview_blob: dict[str, Any] = {
            ...
            "notas_proposta_funcionalidade": True,
            "documento_autonomo_sem_video": bool(documento_autonomo_sem_video),
        }
        steps[CHAVE_STEPS_JSON_PREVIEW_REGENERACAO_TUTORIAL_MARKDOWN_DOCUMENTO_INTEIRO_TRANSCRIBROTHERS] = preview_blob
...
def agendar_regeneracao_markdown_notas_proposta_funcionalidade_em_task_assincrona(
    *,
    job_id: str,
    session_factory: async_sessionmaker[AsyncSession],
    configuracao: ConfiguracaoAmbienteTranscribrothers,
    ...
    asyncio.create_task(
        executar_regeneracao_markdown_notas_proposta_funcionalidade_em_background(
            job_id=job_id,
